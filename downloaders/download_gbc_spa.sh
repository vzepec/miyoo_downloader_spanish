...
filter_spanish() {
  ...
  grep -i -e "EU%29.gbc" -e "Es%2C" -e "%28EU%29" -e "%28Es%29" -e "EU%20-" "$file_to_filter" > temp_files/file_list_filtered.txt
...
wget --limit-rate=0 --tries=3 -q -O - "$BASE_URL4" | grep -o 'href="[^\"]*\.\(gbc\|GBC\)"'  | sed 's/ /%20/g' | sed 's/href="//' | sed 's/"//' > temp_files/file_list_gbc_4.txt
...
cat temp_files/file_list_gbc_4.txt >> temp_files/file_list_gbc.txt
...
sort -u temp_files/file_list_gbc_4.txt -o temp_files/file_list_gbc_4.txt
...
download_filtered_file() {
  ...
  if echo "$line" | grep -q -E '\.gbc$|\.GBC$'; then
    if grep -q "$line" temp_files/file_list_gbc_4.txt; then
      wget -P "../Roms/GBC/" "$BASE_URL4$line"
    else
