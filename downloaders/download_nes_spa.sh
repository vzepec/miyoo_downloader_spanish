...
cat temp_files/file_list_nes_3.txt temp_files/file_list_nes_4.txt >> temp_files/file_list_nes.txt
...
download_filtered_file() {
  ...
  if echo "$line" | grep -q -E '\.nes$|\.NES$'; then
    ...
    if grep -q "$line" temp_files/file_list_nes_3.txt; then
