...
# URL base
BASE_URL="https://archive.org/download/gb-compilacion-de-traducciones-en-espanol_202404/"
BASE_URL2="https://archive.org/download/compilacion-traducciones-en-castellano-gb/"
 
# Crear la carpeta temp_files si no existe
mkdir -p temp_files

# Descargar la lista de archivos para BASE_URL
wget -q -O - "$BASE_URL" | grep -o 'href="[^\"]*\.zip"' | sed 's/ /%20/g' | sed 's/href="//' | sed 's/"//' > temp_files/file_list_gb.txt
...
# Agregar archivos de BASE_URL2 a temp_files/file_list_gb.txt
...
extract_zip() {
  ...
  if [ -n "$invalid_files" ]; then
    find "../Roms/GB" ! -name "*.gb" -type f -delete
    find "../Roms/GB" ! -name "*.gb" -type d -delete
  fi
}

# Función para realizar la sustitución
perform_substitution() {
  echo "$1" | sed -e 's/%20/ /g' -e 's/%28/(/g' -e 's/%29/)/g' -e 's/%2C/,/g' -e 's/%26/\&/g' -e 's/%27/'"'"'/g' -e 's/%21/!/g' -e 's/%25/%/g' -e 's/%5B/[/g' -e 's/%5D/]/g' -e 's/%2B/+/g' -e 's/%C3%AD/í/g' -e 's/%C3%B3/ó/g' -e 's/%C3%B1/ñ/g'
}
...
download_filtered_file() {
  ...
  if grep -q "$line" temp_files/file_list_gb_2.txt; then
    wget -P "../Roms/GB/" "$BASE_URL2$line"
  else
    wget -P "../Roms/GB/" "$BASE_URL$line"
  fi
  file_name=$(perform_substitution "$line")
